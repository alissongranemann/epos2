...
void Cortex_M_IC::hard_fault(const Interrupt_Id & i)
{
    ...
    switch(Traits<Cortex_M_IC>::ACTION_ON_HARD_FAULT) {
        case Traits<Cortex_M_IC>::NOTHING:
            break;
        case Traits<Cortex_M_IC>::REBOOT:
            db<IC>(INF) << "rebooting the machine due to hard fault" << endl;
            for(volatile unsigned int i = 0; i < 0xffff; i++);
            Machine::reboot();
            break;
        case Traits<Cortex_M_IC>::HALT:
            db<IC>(INF) << "halting the machine due to hard fault" << endl;
            CPU::halt();
            break;
    }
}
