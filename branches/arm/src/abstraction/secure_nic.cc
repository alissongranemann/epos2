...
//{
//    db<Secure_NIC>(TRC) << "send(id=" << id << ", data=";
//       for(unsigned int i=0; i<size; i++) 
//           db<Secure_NIC>(TRC) << data[i];
//    db<Secure_NIC>(TRC) << ", size=" << size << endl;
//
...
//
//	// Encrypt the message
//	if(!_encrypt(msg+1, msg+1, size, dst))
//		return -3;
//
...
//{
//    db<Secure_NIC>(TRC) << "send(dst=" << dst << ", data=";
//       for(unsigned int i=0; i<size; i++) 
//           db<Secure_NIC>(TRC) << data[i];
//    db<Secure_NIC>(TRC) << ", size=" << size << endl;
//
...
//		auto val = _keydb->validate_peer(sn, ms, signed_msg, dst);
//        if(Traits<Secure_NIC>::USE_FLASH && val) {
...
//		auto val = _keydb->remove_peer(ms, dst);
//        if(Traits<Secure_NIC>::USE_FLASH && val) {
//            //_keydb->save(Traits<Secure_NIC>::FLASH_ADDRESS);
//        }
//		return false;
...
//            break;
//        case DH_REQUEST:
//            if((!_is_gateway) || (!accepting_connections) || (size != (PUBLIC_KEY_SIZE + 1)))
//                break;
//            //_authenticating = true;
//            // Send back Diffie-Hellman public key
...
//            flag = _keydb->insert_peer((char *)aux, src);
//            if(Traits<Secure_NIC>::USE_FLASH && flag)
//                _keydb->save(Traits<Secure_NIC>::FLASH_ADDRESS);
//            break;
//        case DH_RESPONSE:
//            //if((!_waiting_dh_response) || (size != (PUBLIC_KEY_SIZE + 1))) 
//            //    break;
//            if(size != (PUBLIC_KEY_SIZE + 1))
//                break;
