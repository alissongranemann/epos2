...
template<class T>
class Agent_Common<T, Configurations::EPOS_SOC_Catapult, true>:
...
{
public:
    Agent_Dummy(Channel_t &_rx_ch, Channel_t &_tx_ch, unsigned int inst_id):
            _rx_ch(_rx_ch), _tx_ch(_tx_ch) {}
...
void HW_NODE(name) (Implementation::Channel_t &rx_ch, Implementation::Channel_t &tx_ch, unsigned int inst_id) {\
