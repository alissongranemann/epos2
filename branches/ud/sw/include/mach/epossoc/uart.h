// EPOS EPOSSOC UART Mediator Declarations

#ifndef __epossoc_uart_h
#define __epossoc_uart_h
...
class EPOSSOC_UART: protected UART_Common
{
private:
    ...
    static const unsigned int CLOCK = Traits<EPOSSOC_UART>::CLOCK;
    static const unsigned int BASE_ADDRESS = Traits<EPOSSOC_UART>::BASE_ADDRESS;
	static const unsigned int BAUDRATE = Traits<EPOSSOC_UART>::BAUDRATE;

public:
    EPOSSOC_UART(unsigned int unit = 0) : _unit(unit) { }
    EPOSSOC_UART(unsigned int baud, unsigned int data_bits, unsigned int parity,
