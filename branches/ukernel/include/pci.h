...
public:
    ...
    struct Header {
        ...
        */
        ...
        friend Debug & operator<<(Debug & db, const Header & h) {
            db << h.locator << endl;
            db << "={vnd_id=" << (void *)(int)h.vendor_id << endl;
            db << "dev_id=" << (void *)(int)h.device_id << endl;
            db << "cmd=" << (void *)(int)h.command << endl;
            db << "stat=" << (void *)(int)h.status << endl;
            db << "rev=" << (void *) (int) h.revision_id << endl;
            db << "class_prog=" << (void *) (int) h.class_prog << endl;
            db << "class_id=" << (void *) (int) h.class_id << endl;
            db << "cache_line_size=" << (void *) (int) h.cache_line_size << endl;
            db << "latency_timer=" << (void *) (int) h.latency_time << endl;
            db << "type=" << (void *) (int) h.type << endl;
            db << "bist=" << (void *) (int) h.bist;

            for(unsigned int i = 0; i < Region::N; i++)
                db << "reg[" << i << "]=" << h.region[i] << endl;

            db << "cardbus_cis=" << (void *) (int) h.cardbus_cis << endl;
            db << "sub_vnd_id=" << (void *) (int) h.subsystem_vendor_id << endl;
            db << "sub_dev_id=" << (void *) (int) h.subsystem_device_id << endl;
            db << "rom_addr=" << (void *) (int) h.rom_address << endl;
            db << "interrupt_line=" << (void *) (int) h.interrupt_line << endl;
            db << "interrupt_pin=" << (void *) (int) h.interrupt_pin << endl;
            db << "min_gnt=" << (void *) (int) h.min_gnt << endl;
            db << "max_lat=" << (void *) (int) h.max_lat << endl;
