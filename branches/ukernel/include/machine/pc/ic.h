...
public:
    ...
    /* Reference: Intel Arch Dev Manual Vol 3A, pg 357 PDF, June 2014. */
    static bool is_bsp()
    {
        Reg64 ia32_apic_base_msr = CPU::rdmsr(IA32_APIC_BASE_MSR);

        db<PC_IC>(TRC) << "ia32_apic_base_msr: " << reinterpret_cast<void *>(ia32_apic_base_msr) << endl;

        return (ia32_apic_base_msr & BSP_MASK);
    }
