...
void PC_FPGA::init()
{
    ...
    _dma_write_buffer = new (SYSTEM) DMA_Buffer(DMA_BUFFER_SIZE);
    _dma_read_buffer = new (SYSTEM) DMA_Buffer(DMA_BUFFER_SIZE);

    db<void>(WRN) << "_dma_write_buffer: " << reinterpret_cast<void *>(_dma_write_buffer) << " : " << *_dma_write_buffer << endl;
    db<void>(WRN) << "_dma_read_buffer: " << reinterpret_cast<void *>(_dma_read_buffer) << " : " << *_dma_read_buffer << endl;
